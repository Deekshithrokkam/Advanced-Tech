...
import { STATS, MILESTONES } from '../utils/constants'
import { Trophy, Users, BookOpen, Rocket } from 'lucide-react'

const STAT_ICONS = [Users, BookOpen, Rocket, Trophy]

function StatCounter({ stat, icon: Icon, index, visible }) {
  const count = useCounter(stat.value, 2200, visible)
...
function TimelineItem({ milestone, index, visible }) {
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-5 md:px-8">
        ...
        >
          ...
          <p className="text-zinc-500 max-w-xl mx-auto">
            From founding to our first awards — a snapshot of what ATC has built in a short time.
          </p>
        </motion.div>
        ...
        <div ref={statsRef} className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-20">
          {STATS.map((stat, i) => (
            <StatCounter key={stat.label} stat={stat} icon={STAT_ICONS[i]} index={i} visible={statsVisible} />
          ))}
        </div>
        ...
        >
          <h3 className="text-center text-xl font-bold text-white mb-10">
            Our <span className="text-red-600">Story</span> So Far
          </h3>
        </motion.div>

        <div ref={timeRef} className="flex flex-col gap-6 max-w-2xl mx-auto">
          {MILESTONES.map((m, i) => (
            <TimelineItem key={m.title} milestone={m} index={i} visible={timeVisible} />
          ))}
        </div>
      </div>
    </section>
